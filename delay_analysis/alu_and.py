...
fp3 = open("and_output.txt",'w')
fp3.close()

command = "ngspice and_dest.ckt"

for j in range(0,8): #jfor input.
    if j<4:
        s1 = "A"+str(j)  
    else:
        s1 = "B"+str(j-4) 
    ...
    for i in range(0,4):  #i is for output sequence
        ...
        if(j<4):
            fp1 = open("alu_and1.cir",'r')
        else:
            fp1 = open("alu_and2.cir",'r') 

        fp2 =open("and_dest.ckt",'w') #destination file 
        fp3 = open("and_output.txt",'a') #output file
        mode1 = "RISE"
        mode2 = "RISE"
        mode3 = "FALL"
        mode4 = "FALL"
